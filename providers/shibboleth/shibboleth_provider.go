...
// SetRuntimeConfig installs process-level SAML paths and callback settings.
// The server calls this separately from LoadConfig so request JSON can never
// influence filesystem paths.
func (s *SProvider) SetRuntimeConfig(config RuntimeConfig) {
	s.shibClient.runtimeConfig = config
}
...
func (s *SProvider) GenerateToken(jsonInput map[string]string) (model.Token, int, error) {
	inputMap := strings.TrimSpace(jsonInput["code"])
	if inputMap == "" {
		return model.Token{}, http.StatusBadRequest, fmt.Errorf("SAML assertion attributes are required")
	}
	if s == nil || s.shibClient == nil {
		return model.Token{}, http.StatusServiceUnavailable, fmt.Errorf("SAML provider is not configured")
	}

	var samlData map[string][]string
	if err := json.Unmarshal([]byte(inputMap), &samlData); err != nil {
		log.Warn("SAML assertion attributes could not be decoded")
		return model.Token{}, http.StatusBadRequest, fmt.Errorf("invalid SAML assertion attributes")
	}

	accounts, err := s.shibClient.getShibIdentities(samlData)
	if err != nil {
		log.Warnf("SAML identity mapping failed: %v", err)
		return model.Token{}, http.StatusUnauthorized, err
	}
	identities := make([]client.Identity, 0, len(accounts))
	for _, acct := range accounts {
		shibIdentity := client.Identity{Resource: client.Resource{Type: "identity"}}
		if acct.IsGroup {
	...
	if !ok || strings.TrimSpace(user.ExternalId) == "" {
		return model.Token{}, http.StatusUnauthorized, fmt.Errorf("SAML user identity is missing")
	}
	return model.Token{
		Resource:          client.Resource{Type: "token"},
		IdentityList:      identities,
		Type:              TokenType,
		ExternalAccountID: user.ExternalId,
	}, http.StatusOK, nil
}
...
func (s *SProvider) GetRedirectURL() string {
	...
	path := s.shibClient.runtimeConfig.PlatformAPIHost + "/v1-auth/saml/login"
	return path
}
