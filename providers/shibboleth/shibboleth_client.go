...
func (spclient *SPClient) initializeSPClient(configToSet *model.ShibbolethConfig) error {
	...
	if config.SPSelfSignedCert == "" {
		if config.SPSelfSignedCertFilePath != "" {
			certBytes, err := os.ReadFile(config.SPSelfSignedCertFilePath)
			if err != nil {
	...
	if config.SPSelfSignedKey == "" {
		if config.SPSelfSignedKeyFilePath != "" {
			key, err := os.ReadFile(config.SPSelfSignedKeyFilePath)
			if err != nil {
	...
	actURL, err := parseAbsoluteHTTPURL(config.PlatformAPIHost, "platform API host")
	if err != nil {
		return err
	}
	if actURL.RawQuery != "" {
	...
	if idpURL != "" {
		metadataURL, err := parseAbsoluteHTTPURL(idpURL, "IDP metadata URL")
		if err != nil {
			return err
		}
		resp, err := spclient.httpClient().Get(metadataURL.String())
		if err != nil {
	...
	} else if config.IDPMetadataFilePath != "" {
		file, err := os.Open(config.IDPMetadataFilePath)
		if err != nil {
	...
	return &http.Client{
		...
		CheckRedirect: func(req *http.Request, via []*http.Request) error {
			...
			if len(via) > 0 && via[len(via)-1].URL.Scheme == "https" && req.URL.Scheme != "https" {
				return fmt.Errorf("IDP metadata redirect cannot downgrade HTTPS")
			}
			return nil
		},
	}
