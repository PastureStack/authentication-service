...
func newMockProvider(t *testing.T) *mockProvider {
	...
	t.Setenv(outbound.AllowedOriginsEnvironment, mock.server.URL)
...
func TestRejectsUnauthorizedDiscoveryOriginBeforeNetwork(t *testing.T) {
	requests := 0
	...
	provider, _ := InitializeProvider()
	config := model.AuthConfig{Provider: Config, OIDCConfig: model.OIDCConfig{
		WellKnownURL: server.URL + "/.well-known/openid-configuration",
		ClientID:     "client-id", ClientSecret: "client-secret", Scopes: "openid", UsePKCE: true,
	}}
	if err := provider.LoadConfig(&config); err == nil || !strings.Contains(err.Error(), outbound.AllowedOriginsEnvironment) {
		t.Fatalf("unauthorized discovery origin was accepted: %v", err)
	}
	if requests != 0 {
		t.Fatalf("unauthorized discovery origin received %d requests", requests)
	}
}

func TestRejectsDiscoveryAdvertisedUnauthorizedEndpointBeforeRequest(t *testing.T) {
	attackerRequests := 0
	...
	trusted := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		writeJSON(w, map[string]interface{}{
			"issuer":                                trustedURL,
			"authorization_endpoint":                attacker.URL + "/authorize",
			"token_endpoint":                        attacker.URL + "/token",
			"userinfo_endpoint":                     attacker.URL + "/userinfo",
	...
	provider, _ := InitializeProvider()
	config := model.AuthConfig{Provider: Config, OIDCConfig: model.OIDCConfig{
		WellKnownURL: trusted.URL + "/.well-known/openid-configuration",
		ClientID:     "client-id", ClientSecret: "client-secret", Scopes: "openid", UsePKCE: true,
	}}
	if err := provider.LoadConfig(&config); err == nil || !strings.Contains(err.Error(), "not authorized") {
		t.Fatalf("unauthorized discovery endpoint was accepted: %v", err)
	}
	if attackerRequests != 0 {
		t.Fatalf("advertised attacker origin received %d requests", attackerRequests)
	}
}

func TestAcceptsExplicitlyAuthorizedMultiOriginDiscovery(t *testing.T) {
	...
	discovery := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		writeJSON(w, map[string]interface{}{
			"issuer":                                endpoint.URL,
			"authorization_endpoint":                endpoint.URL + "/authorize",
			"token_endpoint":                        endpoint.URL + "/token",
			"userinfo_endpoint":                     endpoint.URL + "/userinfo",
			"jwks_uri":                              endpoint.URL + "/jwks",
			"token_endpoint_auth_methods_supported": []string{"client_secret_basic"},
			"id_token_signing_alg_values_supported": []string{"RS256"},
		})
	}))
	defer discovery.Close()
	t.Setenv(outbound.AllowedOriginsEnvironment, discovery.URL+","+endpoint.URL)
	provider, _ := InitializeProvider()
	config := model.AuthConfig{Provider: Config, OIDCConfig: model.OIDCConfig{
		WellKnownURL: discovery.URL + "/.well-known/openid-configuration",
		ClientID:     "client-id", ClientSecret: "client-secret", Scopes: "openid", UsePKCE: true,
	}}
	if err := provider.LoadConfig(&config); err != nil {
		t.Fatalf("explicitly authorized multi-origin discovery was rejected: %v", err)
	}
	redirect, err := url.Parse(provider.GetRedirectURL())
	if err != nil || redirect.String() == "" || !strings.HasPrefix(redirect.String(), endpoint.URL+"/authorize") {
		t.Fatalf("unexpected multi-origin authorization URL %q: %v", redirect, err)
	}
}

func TestOIDCRedirectCannotLeaveAuthorizedOrigin(t *testing.T) {
	attackerRequests := 0
	attacker := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		attackerRequests++
		writeJSON(w, map[string]interface{}{})
	...
	defer trusted.Close()
	policy, err := outbound.NewOriginPolicy(trusted.URL, attacker.URL)
	if err != nil {
		t.Fatal(err)
	}
	client, err := newHTTPClient("", policy)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := client.Get(trusted.URL + "/start"); err == nil {
		t.Fatal("cross-origin OIDC redirect was accepted")
	}
	if attackerRequests != 0 {
...
func TestJWKSRequestRechecksAuthorizedOriginBeforeNetwork(t *testing.T) {
	requests := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests++
	...
	defer server.Close()
	policy, err := outbound.NewOriginPolicy("https://trusted.example")
	if err != nil {
		t.Fatal(err)
	}
	client := &Client{
		discovery:    discoveryDocument{JWKSURI: server.URL + "/jwks"},
		originPolicy: policy,
		httpClient:   server.Client(),
	}
	if _, err := client.getSigningKeys(t.Context(), false); err == nil || !strings.Contains(err.Error(), "not authorized") {
		t.Fatalf("unauthorized JWKS origin was accepted: %v", err)
	}
	if requests != 0 {
		t.Fatalf("unauthorized JWKS origin received %d requests", requests)
	}
}
