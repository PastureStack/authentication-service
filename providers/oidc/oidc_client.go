...
func (c *Client) initialize() error {
	...
	if !originPolicy.IsValidRedirectURL(c.config.WellKnownURL) {
		return fmt.Errorf("OIDC discovery origin is not authorized by %s", outbound.AllowedOriginsEnvironment)
	}
	c.originPolicy = originPolicy

	httpClient, err := newHTTPClient(c.config.CertificateAuthority, originPolicy)
	if err != nil {
		return err
	}
...
func newHTTPClient(customCA string, originPolicy *outbound.OriginPolicy) (*http.Client, error) {
	...
	client := &http.Client{
		Transport: &outbound.PolicyTransport{Base: transport, Policy: originPolicy},
	...
	client.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if len(via) == 0 || !outbound.SameOrigin(via[0].URL, req.URL) {
			return fmt.Errorf("OIDC endpoint redirect changed the authorized origin")
		}
		if originPolicy == nil || !originPolicy.IsValidRedirectURL(req.URL.String()) {
			return fmt.Errorf("OIDC endpoint redirect target is not authorized")
		}
		return nil
	}
	return client, nil
...
func (c *Client) fetchDiscovery(ctx context.Context) error {
	...
	for name, endpoint := range required {
		...
		if c.originPolicy == nil || !c.originPolicy.IsValidRedirectURL(endpoint) {
			return fmt.Errorf("OIDC discovery %s origin is not authorized by %s", name, outbound.AllowedOriginsEnvironment)
		}
	}
...
func (c *Client) generateToken(input map[string]string) (model.Token, int, error) {
	...
	authenticationMethods := stringValues(claimValue(claimMap, "amr"))
	authenticationContext := claimString(claimMap, "acr")
	authenticatedAt := integerValue(claimValue(claimMap, "auth_time"))
	...
	token, status, err := c.tokenFromClaims(tokenResponse.AccessToken, claimMap)
	if err != nil {
		return model.Token{}, status, err
	}
	token.AuthenticationMethods = authenticationMethods
	token.AuthenticationContext = authenticationContext
	token.AuthenticatedAt = authenticatedAt
	token.AuthenticationIssuer = c.discovery.Issuer
...
func (c *Client) exchangeCode(ctx context.Context, response authorizationResponse) (tokenEndpointResponse, error) {
	...
	if c.originPolicy == nil || !c.originPolicy.IsValidRedirectURL(c.discovery.TokenEndpoint) {
		return tokenEndpointResponse{}, fmt.Errorf("OIDC token endpoint origin is not authorized")
	}
...
func (c *Client) getJSON(ctx context.Context, endpoint string, bearerToken string, destination interface{}) error {
	if c.originPolicy == nil || !c.originPolicy.IsValidRedirectURL(endpoint) {
		return fmt.Errorf("OIDC endpoint origin is not authorized")
	}
