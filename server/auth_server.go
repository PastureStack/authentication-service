...
func initProviderWithConfig(authConfig *model.AuthConfig) (providers.IdentityProvider, error) {
	...
	if authConfig.Provider == "shibbolethconfig" {
		shibbolethProvider, ok := newProvider.(*shibboleth.SProvider)
		if !ok {
			return nil, fmt.Errorf("Could not configure the Shibboleth runtime")
		}
		shibbolethProvider.SetRuntimeConfig(shibboleth.RuntimeConfig{
			IDPMetadataFilePath:      IDPMetadataFile,
			SPSelfSignedCertFilePath: selfSignedCertFile,
			SPSelfSignedKeyFilePath:  selfSignedKeyFile,
			PlatformAPIHost:          GetPlatformAPIHost(),
		})
	}
