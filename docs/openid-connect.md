...
The server operator must also list the exact discovery and advertised endpoint
origins in `PASTURESTACK_AUTH_ALLOWED_EXTERNAL_ORIGINS` before configuration.
Use a comma-separated list of origins without paths, queries, credentials, or
fragments. This is intentionally an operator-side boundary: a provider request,
discovery document, redirect, or compromised administrator browser cannot add
an outbound destination. Providers that publish authorization, token,
UserInfo, and JWKS endpoints on multiple origins remain supported when each
origin is explicitly listed. Redirects must stay on the original request
origin even when both origins are separately authorized.
